...
int main(int argc, char *argv[])
{
    ...
    for (const auto &pair : countUniqueStones25)
    {
        ...
        for (auto uniqueStone50 : countUniqueStones50) {
            if (countUniqueStoneFor75Blink.count(uniqueStone50.first) > 0) {
                countUniqueStoneFor75Blink[uniqueStone50.first] += uniqueStone50.second * pair.second;
            } else {
                countUniqueStoneFor75Blink[uniqueStone50.first] = uniqueStone50.second * pair.second;
            }
        }
    ...
    for (auto uniqueStone75 : countUniqueStoneFor75Blink)
    {
        // cout << uniqueStone75.first << " " << uniqueStone75.second << endl;
        result += blink25WithSingleStone(uniqueStone75.first) * uniqueStone75.second;
    }
